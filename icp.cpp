...
int main(int argc, char *argv[]) {
  ...
  std::cout << "\nLoaded file " << argv[2] << " (" << cloud_tr->size()
            << " points) in " << time.toc() << " ms\n"
            << std::endl;
  *cloud_icp = *cloud_tr;
  ...
  /* double theta = M_PI / 8;  // The angle of rotation in radians
   ...
   << std::endl; print4x4Matrix (transformation_matrix);
  ...
  PointCloudWithNormal::Ptr cloud_in_normal(new PointCloudWithNormal);
  PointCloudWithNormal::Ptr cloud_icp_normal(new PointCloudWithNormal);
  ...
  norm_est.compute(*cloud_in_normal);
  pcl::copyPointCloud(*cloud_in, *cloud_in_normal);
  norm_est.setInputCloud(cloud_icp);
  norm_est.compute(*cloud_icp_normal);
  pcl::copyPointCloud(*cloud_icp, *cloud_icp_normal);

  pcl::IterativeClosestPointNonLinear<PointNormalT,PointNormalT> icp;
  icp.setMaximumIterations(iterations);
  //icp.setTransformationEpsilon(1e-6);
  //icp.setMaxCorrespondenceDistance (0.1);      
  icp.setInputSource (cloud_icp_normal);  
  icp.setInputTarget (cloud_in_normal); 
  icp.align (*cloud_icp_normal);
  ...
  icp.align(*cloud_icp);*/
  icp.setMaximumIterations(1); // We set this variable to 1 for the next time we
                               // will call .align () function
    ...
    if (next_iteration) {
      ...
      if (icp.hasConverged()) {
        ...
        transformation_matrix *=
            icp.getFinalTransformation().cast<double>(); // WARNING /!\ This is
                                                         // not accurate! For
                                                         // "educational"
                                                         // purpose only!
        print4x4Matrix(transformation_matrix); // Print the transformation
                                               // between original pose and
                                               // current pose
